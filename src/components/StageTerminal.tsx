    ...
    <div className="flex items-end gap-[2px] h-full w-full">
      {[...Array(24)].map((_, i) => (
        <motion.div
          ...
          className="flex-1 bg-brand-gold/40 rounded-full min-w-[2px]"
        />
      ))}
    </div>
  );
    ...
    <div className={`relative glass p-4 md:p-6 rounded-3xl border border-white/20 shadow-2xl overflow-hidden group ${className}`}>
      ...
      <div className="absolute top-0 left-0 right-0 h-10 bg-white/5 border-b border-white/10 flex items-center px-4 justify-between z-10 backdrop-blur-md">
        ...
        <div className="bg-white/5 px-3 py-1 rounded-md border border-white/5">
          <div className="text-[10px] text-white/30 font-mono tracking-widest uppercase">OCASSION_ORBIT_TERMINAL_V1.0</div>
        </div>
      </div>
      ...
      <div className="pt-14 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="glass-card p-4 border-white/5 min-h-[160px] flex flex-col justify-between">
            <div className="flex justify-between items-center mb-4">
              <span className="text-[10px] font-black uppercase tracking-widest text-brand-gold flex items-center gap-2">
          ...
          <div className="glass-card p-4 border-white/5 min-h-[160px] flex flex-col">
            <span className="text-[10px] font-black uppercase tracking-widest text-brand-purple mb-4 flex items-center gap-2">
              <BarChart3 className="w-3 h-3" /> Pitch Analysis
            </span>
            <div className="flex-1 flex flex-col justify-center space-y-4">
              <div className="flex justify-between items-end">
                <span className="text-3xl font-black text-white italic">C#<span className="text-brand-purple">m</span></span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="glass-card p-4 border-white/5 col-span-1 md:col-span-2">
            <div className="flex justify-between items-center mb-4">
              <span className="text-[10px] font-black uppercase tracking-widest text-white/50 flex items-center gap-2">
                <Layers className="w-3 h-3" /> Orbital FX Stack
              </span>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className={`p-2 rounded-lg border flex flex-col ${fx.active ? 'bg-brand-purple/5 border-brand-purple/20' : 'bg-white/[0.02] border-white/5 opacity-40'}`}>
          ...
          <div className="glass-card p-4 border-white/5 flex flex-col justify-between">
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest text-brand-gold mb-4 block">System Status</span>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-white/30 uppercase">Latency</span>
                  <span className="text-[10px] font-mono text-brand-gold">4.2ms</span>
                </div>
                <div className="flex justify-between items-center">
            ...
            <div className="pt-4 mt-4 border-t border-white/5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-brand-gold animate-pulse shadow-[0_0_5px_#D4AF37]" />
                <span className="text-[8px] font-black uppercase text-brand-gold">Sync Active</span>
              </div>
